...
export default function Page() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const result = await res.json().catch(() => ({})); // مهم: اگر بدنه خالی بود، خطا ندهد

      if (res.ok) {
        alert("درخواست شما با موفقیت ارسال شد!"); 
        e.currentTarget.reset();
        return; // خیلی مهم: نذاریم ادامه اجرا شود
      }
  ...
  return (
    <main dir="rtl" style={{ "--accent": accent }} className="min-h-screen bg-[#050505] text-white overflow-x-hidden">
        ...
        <div className="max-w-7xl mx-auto px-8 md:px-16 w-full relative z-10">
          ...
          >
            <motion.div variants={item} className="space-y-8">
              ...
              <motion.h1
                {...fadeUp}
                className="text-white/30 text-3xl md:text-5xl lg:text-6xl leading-[1.1] font-light tracking-tight">
                کسب‌وکاری که
                <br />
                <span className="text-[color:var(--accent)]">پلن مشخص</span> داره
                <br />
                <span className="text-white/70"> برند میشه</span>
              </motion.h1>
              ...
              <div className="relative w-full max-w-[560px] aspect-square">
                ...
                >
                  <div className="flex items-center justify-between">
                    <div className="text-white/60 text-sm">Plan Clarity</div>
                    <div
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                      <div className="text-[10px] text-white/45 uppercase tracking-[0.2em]">جذب</div>
                      <div className="mt-3 h-2 w-full rounded-full bg-white/10" />
                    </div>
                    <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                      <div className="text-[10px] text-white/45 uppercase tracking-[0.2em]">نگهداشت</div>
                      <div className="mt-3 h-2 w-full rounded-full bg-white/10" />
                    </div>
                    <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                      <div className="text-[10px] text-white/45 uppercase tracking-[0.2em]">رشد</div>
                      <div className="mt-3 h-2 w-full rounded-full bg-white/10" />
                    </div>
                  </div>
                </motion.div>
        ...
        <div className="max-w-7xl mx-auto px-8 md:px-16">
          ...
          >
            ...
            <h2 className=" mx-auto text-center text-3xl md:text-5xl lg:text-6xl font-light leading-[1.2] tracking-tight">
              رشد کسب‌و‌کار و برند شدن فرسایشیه
              <br />
              <span className="text-white/30 text-1xl md:text-2xl lg:text-3xl">
                اگه پلن مشخصی برای کسب‌وکارت نداشته باشی
              </span>
            </h2>
            ...
            <ul className="grid grid-cols-1 md:grid-cols-3 gap-10 text-white/80 text-lg leading-8 justify-items-center">
              ...
              <li className="flex gap-4 items-start max-w-[260px]">
                <span className="mt-1 w-3 h-3 rounded-full bg-[#2254f6]" />
                زمان زیادی صرف ایده‌هایی می‌شن که نتیجه‌ی واقعی ندارند
              </li>

              <li className="flex gap-4 items-start max-w-[260px]">
                <span className="mt-1 w-3 h-3 rounded-full bg-[#2254f6]" />
                حتی اگر تیم حرفه‌ای هم بتونی بسازی اون تیم درگیر خطا و آزمون‌های بی انتها میشه
              </li>

              <li className="flex gap-4 items-start max-w-[260px]">
                <span className="mt-1 w-3 h-3 rounded-full bg-[#2254f6]" />
                هزینه‌های زیادی صرف تبلیغات اشتباه میشه
              </li>
      ...
      <section id="services" className="py-12 border-t border-white/5">
        {/* عرض کانتینر مشابه بخش‌های دیگر */}
        <div className="max-w-7xl mx-auto px-8 md:px-16">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {[
              {
                title: "تولید‌محتوای هدف‌مند",
                text: "تولید محتوا صرفا در مورد تجهیزات و تکنیک‌های ترند نیس، متناسب با هویت برند شما.",
                linkText: "تولید محتوا در اینستا >",
                href: "/contentCreation"
              },
              {
                title: "تبلیغات اصولی",
                text: "ساخت صفحه و مسیر ارائه‌ای که مخاطب را تا اقدام نهایی همراه کند.",
                linkText: "تبلیغات در بله >",
                href: "/advertising"
              },
              {
                title: "طراحی سایت چابک",
                text: "تعریف دقیق ارزش پیشنهادی، مخاطب هدف و جمله‌ای که برند را واضح معرفی کند.",
                linkText: "طراحی سایت >",
                href: "/web-design"
              },
            ].map((card, index) => (
              <motion.div
                ...
                className="rounded-3xl border border-white/10 bg-white/5 p-8 flex flex-col h-full"
              >
                <div
                ...
                {/* متن که flex-grow می‌گیرد تا فاصله را پر کند */}
                <p className="text-white/55 leading-8 mb-8 flex-grow">
                  {card.text}
                </p>
                
                {/* لینک پایین کارت */}
                <a href={card.href} className="text-sm font-medium opacity-80 hover:opacity-100 transition-opacity">
                  {card.linkText}
                </a>
              </motion.div>
            ))}
